...
You also should have the relevant tools installed to generate the server and client interface code - if you don’t already, follow the setup instructions in [Quick start]({{< ref "/docs/Languages/Node/Quickstart" >}}).

### Defining the service

Our first step (as you’ll know from the [Introduction to gRPC]({{< ref "/docs/WhatisgRPC/Introduction" >}})) is to define the gRPC *service* and the method *request* and *response* types using [protocol buffers](https://protobuf.dev/overview). You can see the complete .proto file in [`examples/protos/route_guide.proto`](https://github.com/grpc/grpc-node//blob/@grpc/grpc-js@1.9.0/examples/protos/route_guide.proto).
